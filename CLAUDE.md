...
- One-shot central web UI for verification: `make web-dev` brings up a dedicated `rex-pg-dev` Postgres container on port 55433 and runs `rex-central serve --dev` (registers the local default identity at `~/.config/rex/identity/` as an authorized key, promotes it to admin of the default org, auto-enables `--web`). Open http://127.0.0.1:8080/login, then `bin/rex remote login dev http://127.0.0.1:8080` from another terminal to set a session cookie. `make web-dev-down` tears down the pg container. NEVER pass `--dev` in production.
